            ...
            return (
              <Card
                ...
                className={`p-6 hover:shadow-xl transition-all duration-500
                   hover:-translate-y-2 bg-gradient-to-br ${instrument.gradient} border-2 ${instrument.borderColor} ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
                  }`}
                style={{ transitionDelay: `${index * 100}ms` }}
              >
                <div className={`w-12 h-12 rounded-full ${instrument.iconBg} flex items-center justify-center mb-4`}>
                  <Icon className={`w-6 h-6 ${instrument.iconColor} dark:text-white`} />
                ...
                <div className={`text-xs font-medium ${instrument.iconColor} dark:text-white`}>
                  {instrument.years}
                </div>
